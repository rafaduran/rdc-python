...
class MainForm(QDialog):
        ...
        for text, slot in (
            ...
            if text == "&Quit":
                ...
                for text, arg in (
                        ...
                        ("Align &Botton", Qt.AlignBottom)):
                    func = functools.partial(self.align, arg)
                    menu.addAction(QString(text), func)
                button.setMenu(menu)
    ...
    def align(self, align_type):
        """
        align(self, alig_type)
            align objects to the given algin_type aligment
        align_type:
            QtCore aligment (Qt.AlignLeft, Qt.Align.Right,...)
        """
        items = self.scene.selectedItems()
        if items is None or len(items) < 1:
            return
        item_rects = [item.sceneBoundingRect() for item in items]
        #positions = [item.scenePos() for item in items]
        if align_type == Qt.AlignLeft:
            xpos = min([rect.x() for rect in item_rects])
            [item.moveBy(xpos - item.sceneBoundingRect().x(), 0) for item in 
                    items]
        elif align_type == Qt.AlignRight:
            xpos = max([rect.x() + rect.width() for rect in item_rects])
            [item.moveBy(xpos - item.sceneBoundingRect().x() -
                item.sceneBoundingRect().width(), 0) for item in items]
        elif align_type == Qt.AlignTop:
            ypos = min([rect.y() for rect in item_rects])
            [item.moveBy(0, ypos - item.sceneBoundingRect().y()) for item in 
                    items]
        elif align_type == Qt.AlignBottom:
            ypos = max([rect.y() + rect.height()  for rect in item_rects])
            [item.moveBy(0, item.sceneBoundingRect().height() + 
                item.sceneBoundingRect().y() - ypos) for item in items]
        else:
            raise ValueError("Invalid aligment")
        global Dirty
        Dirty = True
