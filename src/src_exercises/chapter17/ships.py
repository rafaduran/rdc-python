...
class ShipContainer(object):
    ...
    def in_teu_order(self):
        return sorted(self.ships.values(), key = lambda x:
                (x.replace(QRegExp("[., ]"), "").toInt()[0]))
    ...
    def load(self):
        ...
        try:
            if self.filename.isEmpty():
                raise IOError, \
                    QApplication.translate("no filename specified for loading")
            ...
            if magic != MAGIC_NUMBER:
                raise IOError, QApplication.translate("unrecognized file type")
            fileVersion = stream.readInt16()
            if fileVersion != FILE_VERSION:
                raise IOError, \
                    QApplication.translate("unrecognized file type version")
            self.ships = {}
    ...
    def save(self):
        ...
        try:
            if self.filename.isEmpty():
                raise IOError, \
                    QApplication.translate("no filename specified for saving")
            fh = QFile(self.filename)
...
class ShipTableModel(QAbstractTableModel):
    ...
    def sort_by_teu(self):
        self.ships = sorted(self.ships, key = lambda x: (x.teu))
        self.reset()
    ...
    def data(self, index, role=Qt.DisplayRole):
        ...
        if role == Qt.DisplayRole:
            ...
            if column == NAME:
                ...
                return QString(unicode(ship.teu)) + \
                    self.tr(" twenty foot equivalents")
        return QVariant()
    ...
    def headerData(self, section, orientation, role=Qt.DisplayRole):
        ...
        if orientation == Qt.Horizontal:
            if section == NAME:
                ...
                return QVariant(self.tr("Owner"))
            elif section == COUNTRY:
                return QVariant(self.tr("Country"))
            elif section == DESCRIPTION:
                return QVariant(self.tr("Description"))
            elif section == TEU:
                return QVariant(self.tr("TEU"))
    ...
    def load(self):
        ...
        try:
            if self.filename.isEmpty():
                raise IOError, self.tr("no filename specified for loading")
            fh = QFile(self.filename)
            if not fh.open(QIODevice.ReadOnly):
                raise IOError, unicode(fh.errorString())
            stream = QDataStream(fh)
            magic = stream.readInt32()
            if magic != MAGIC_NUMBER:
                raise IOError, self.tr("unrecognized file type")
            fileVersion = stream.readInt16()
            if fileVersion != FILE_VERSION:
                raise IOError, self.tr("unrecognized file type version")
            self.ships = []
            while not stream.atEnd():
                name = QString()
                owner = QString()
    ...
    def save(self):
        ...
        try:
            if self.filename.isEmpty():
                raise IOError, self.tr("no filename specified for saving")
            fh = QFile(self.filename)
            if not fh.open(QIODevice.WriteOnly):
                raise IOError, unicode(fh.errorString())
            stream = QDataStream(fh)
    ...
    def export(self, filename):
        ...
        """
        if filename is None:
            raise ValueError(self.tr("Invalid filename"))
        fh = None
        try:
            fh = QFile(filename)
            if not fh.open(QIODevice.WriteOnly):
